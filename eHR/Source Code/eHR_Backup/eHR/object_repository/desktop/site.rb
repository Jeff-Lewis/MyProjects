  ...
  class Site
    ...
    def create_site(str_data_node = "site_data")
      begin
        ...
        self.textfield_site_code = ""     # clear all textboxes - required for re-iterating if 'Site with same code already exists' occurs
        self.textfield_site_name = ""
        self.textarea_contact_info = ""
        ...
        if !is_text_present(self, "Site created successfully", 5)
          if is_text_present(self, "Site with same code already exists",3)
            create_site
          else
            raise "Failure in creating site"
          end
        end
