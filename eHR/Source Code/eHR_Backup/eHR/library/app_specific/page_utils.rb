    ...
    def close_application_windows
      begin
        ...
        if link_close_health_status_iframe_element.exists? && link_close_health_status_iframe_element.visible?
          # as of now commented as patient, visit iframes also has the same property and will be closed if opened
          #div_health_status_iframe_header = @browser.find_element(:id, "popUpNonCompHealthStatus_h")
          #@browser.action.drag_and_drop_by(div_health_status_iframe_header, -100, 300).perform
