  ...
  module FileLibrary
    ...
    def get_files_absolute_path(str_file_path, str_file_type, obj_time_stamp)
      begin
        ...
        arr_files = []
        $log.success("Object time stamp => #{obj_time_stamp}")
        #arr_abs_path.each do |file_path|
        #  arr_files << file_path if File.ctime(file_path) > obj_time_stamp
        #end
        arr_abs_path.each do |file_path|
          if File.ctime(file_path) > obj_time_stamp
            arr_files << file_path
            $log.success("File path => #{file_path}, File creation time => #{File.ctime(file_path)}")
            $log.success("Array size => #{arr_files.size}")
          end
        end
        $log.success("Array size => #{arr_files.size}")
        arr_files
