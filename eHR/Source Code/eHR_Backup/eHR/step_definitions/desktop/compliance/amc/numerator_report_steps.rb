...
Then /^"(.*?)" should (?:be|not be) in "(.*?)" numerator report under "(.*?)"$/ do |str_record, str_objective, str_table|
  ...
  on(EHR::AMCStage) do |page|
    ...
    if str_record.downcase == "a record" && !bool_record
      $log.success("The patient id #{@str_patient_id} with record for #{arr_visit_id.join(", ") if !arr_visit_id.nil?} and #{str_reason} exist in the Report")
      ...
      begin
        @num_numerator_count = page.click_numerator(str_table, str_objective)
        raise "No such Patient id (#{@str_patient_id}) with record for #{arr_visit_id.join(", ") if !arr_visit_id.nil?} and #{str_reason} exists in the report" if !page.is_report_exists(@str_patient_id, arr_visit_id, str_reason)
      rescue Exception => ex
        if $scenario_tags.include? "@hl7"
          ...
          #raise ex
          $log.error("No such Patient id (#{@str_patient_id}) with record for #{arr_visit_id.join(", ") if !arr_visit_id.nil?} and #{str_reason} exists in the report")  # raise ex is not working sometimes, so raise is changed by $log.error
          exit
        end
      end
    elsif arr_record.include?(str_record.downcase) && bool_record
      $log.success("The patient id #{@str_patient_id} with record for #{arr_visit_id.join(", ") if !arr_visit_id.nil?} and #{str_reason} does not exist in the Report")
    elsif arr_record.include?(str_record.downcase) && !bool_record
      raise "The patient id #{@str_patient_id} with record for #{arr_visit_id.join(", ") if !arr_visit_id.nil?} and #{str_reason} should not exist in the Report"
    end
