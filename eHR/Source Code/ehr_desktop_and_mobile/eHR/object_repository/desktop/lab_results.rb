...
module EHR
  module OrderEntryTab
    module LabResults
      ...
      #
      def create_lab_result(str_affirmation, str_report_date, str_lab_result_node = "lab_results_data1")
        begin
          if $scenario_tags.include?("@tc_755")
            $num_test_data_count += 1
            str_lab_result_node = "lab_results_data#{$num_test_data_count}"
          end
          ...
          str_test_performed_half_name = hash_lab_result[str_lab_result_node]["textfield_test_performed_half_name"]
          str_test_performed_full_name = hash_lab_result[str_lab_result_node]["textfield_test_performed_full_name"]
          ...
          if textfield_lab_name == ""
            ...
            div_ajax_lab_name_element.list_item_elements(:xpath => ".//ul/li").each do |list_item|
              list_item.scroll_into_view rescue Exception
              if list_item.text.downcase.strip == str_lab_name.downcase
                list_item.click
                break
              end
            end
            ...
            self.textfield_test_performed = str_test_performed_half_name
            wait_for_object(div_ajax_test_performed_element)
            div_ajax_test_performed_element.list_item_elements(:xpath => ".//ul/li").each do |list_item|
              list_item.scroll_into_view rescue Exception
              if list_item.text.downcase.strip == str_test_performed_full_name.downcase
                list_item.click
                break
              end
            end
